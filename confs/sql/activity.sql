...
--select usename, application_name, pid, query_start, state_change, state, wait_event_type, wait_event, query from pg_stat_activity where (wait_event_type <> 'Client' or wait_event_type is null or state is null or state='active');
select usename, application_name, pid, query_start, state_change, wait_event_type, wait_event, state, query from pg_stat_activity; where application_name='sciz-w';
VACUUM ANALYZE ;
-- analyse;
-- show autovacuum;
-- select now();
...
-- select pg_cancel_backend(31131);
/* autovacuum info */
WITH raw_data AS (
  SELECT
